...
function clientNav(screen, tab) {
  ...
  if (screen === 'log') {
    if (!isCohortLive()) {
      showToast('Sessions unlock on cohort start date.');
      return;
    }
    loadLogScreen();
  }
  ...
  if (screen === 'progress') {
    if (!isCohortLive()) {
      showToast('Progress tracking unlocks on 31 August when the cohort begins.');
      clientNav('home', document.querySelector('#client-nav .nav-tab'));
      return;
    }
...
async function loadClientHome() {
  ...
  // Check cohort gate — show countdown if programme not yet started
  if (checkCohortGate()) {
    showCountdownScreen();
    return;
  }
...
function intakeNext() {
  ...
  document.getElementById(`ip${intakeCur}`)?.classList.remove('active');
  intakeCur = next;
  document.getElementById(`ip${intakeCur}`)?.classList.add('active');
  ...
  // Update next button text on last phase
...
async function submitIntake() {
  const btn = document.getElementById('intake-next-btn');
  btn.disabled = true;
  btn.textContent = 'Submitting...';

  const email = iv('if_email') || currentUser?.email || '';

  // Measurement fields (Phase 11)
  const measurementLabels = ['chest','waist','hips','left_arm','right_arm','left_thigh','right_thigh','neck'];
  const measurements = {};
  measurementLabels.forEach((label, i) => {
    const val = parseFloat(document.getElementById(`if_m_${i}`)?.value);
  ...
  const baselineData = {
    ...
    phone:             iv('if_phone'),
    date_of_birth:     iv('if_dob'),
    sex:               intakeSex,
    weight_kg:         parseFloat(iv('if_weight')) || null,
    height_cm:         parseFloat(iv('if_height_cm')) || null,
    body_fat_pct:      parseFloat(iv('if_bf')) || null,
    target_weight_kg:  parseFloat(iv('if_target_weight')) || null,
    resting_hr:        parseInt(iv('if_rhr')) || null,
    pullup_max:        parseInt(iv('if_pullups')) || null,
    pushup_max:        parseInt(iv('if_pushups')) || null,
    programme_goal:    intakeRadioVals['goal'] || '',
    activity_level:    intakeRadioVals['activity'] || '',
    experience:        intakeRadioVals['experience'] || '',
    current_days:      intakeRadioVals['current_days'] || '',
    meals_per_day:     intakeRadioVals['meals'] || '',
    protein_tracking:  intakeRadioVals['protein'] || '',
    intolerances:      getIntoleranices(),
    sleep_hours:       intakeRadioVals['sleep'] || '',
    commitment_level:  intakeRadioVals['commitment'] || '',
    relationship:      intakeRadioVals['relationship'] || '',
    children:          intakeRadioVals['children'] || '',
    referral_source:   intakeRadioVals['referral'] || '',
    health_check:      intakeRadioVals['health_check'] || '',
    life_stage:        intakeRadioVals['life_stage'] || '',
    pregnancy:         intakeRadioVals['pregnancy'] || '',
    client_type:       intakeChipVals['client_type'] || 'in_person',
    goal_desc:         iv('if_goal_desc'),
    worked:            iv('if_worked'),
    not_worked:        iv('if_not_worked'),
    typical_week:      iv('if_typical_week'),
    injuries:          iv('if_injuries'),
    medications:       iv('if_meds'),
    other_medical:     iv('if_other_medical'),
    support_person:    iv('if_support_person'),
    need_from_mike:    iv('if_need_from_mike'),
    why:               iv('if_why'),
    vision:            iv('if_vision'),
    final_notes:       iv('if_final_notes'),
    occupation:        iv('if_occupation'),
    steps_per_day:     parseInt(iv('if_steps')) || null,
    stress_level:      parseInt(iv('if_stress')) || null,
    water_litres:      parseFloat(iv('if_water')) || null,
    cycle_length:      parseInt(iv('if_cycle_length')) || null,
    breakfast:         iv('if_breakfast'),
    female_other:      iv('if_female_other'),
    ...measurements,
    submitted_at:      new Date().toISOString(),
  };

  try {
    // Save to client_baselines
    await fetch(`${SB_URL}/rest/v1/client_baselines`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'apikey': SB_KEY,
    ...
    // Mark intake complete on platform_users
    await fetch(`${SB_URL}/rest/v1/platform_users?email=eq.${encodeURIComponent(email)}`, {
      ...
      body: JSON.stringify({
        ...
        sex: intakeSex,
        client_type: intakeChipVals['client_type'] || 'in_person',
        life_stage: intakeRadioVals['life_stage'] || null,
        cycle_length: parseInt(iv('if_cycle_length')) || null,
      })
    });
    ...
    fetch('https://services.leadconnectorhq.com/hooks/joReyyNlTbY9adnjCSpY/webhook-trigger/fc04e247-953c-4842-a223-77c32f571441', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...baselineData, type: 'intake_form' })
    }).catch(() => {});

    // Launch app directly — do not call initApp() to avoid gate re-check
    console.log('submitIntake: launching app');
    document.getElementById('intake-gate').style.display = 'none';
    document.getElementById('auth-screen').classList.add('hidden');
    document.getElementById('app-shell').classList.remove('hidden');

    var nameStr = (currentUser && currentUser.first_name) ? currentUser.first_name : email.split('@')[0];
    var topbarUser = document.getElementById('topbar-user');
    var topbarAvatar = document.getElementById('topbar-avatar');
    if (topbarUser)   topbarUser.textContent   = nameStr;
    if (topbarAvatar) topbarAvatar.textContent = nameStr[0].toUpperCase();

    var coachScreens  = document.getElementById('coach-screens');
    var clientScreens = document.getElementById('client-screens');
    var coachNav  = document.getElementById('coach-nav');
    var clientNav = document.getElementById('client-nav');
    if (coachScreens)  coachScreens.style.display  = 'none';
    if (clientScreens) clientScreens.style.display = 'block';
    ...
    btn.disabled = false;
    btn.textContent = 'Submit and Enter App →';
    alert('Something went wrong saving your intake. Please try again.');
  }
}
